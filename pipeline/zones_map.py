...
from src.constants import ZONES  # noqa: E402
...
ZONE_COL = {"teso_kyoga": "#1f77b4", "elgon": "#b2182b", "karamoja": "#e08214", "adjumani": "#5e3c99"}
HATCH = ["///", "\\\\\\", "xxx", "..."]
...
def main() -> None:
    ...
    try:
        ...
        im = ax.imshow(rec, extent=(b.left, b.right, b.bottom, b.top), cmap="Blues", vmin=0, vmax=1,
                       alpha=0.85, zorder=1)
        ...
        ax.text(0.01, 0.01, f"(recurrence backdrop unavailable: {e})", transform=ax.transAxes, fontsize=7)
    ...
    for key, zone in ZONES.items():
        ...
        col = ZONE_COL[key]
        g[g.membership == "candidate"].plot(ax=ax, aspect=None, color=col, alpha=0.18, edgecolor=col, linewidth=0.6, zorder=3)
        g[g.membership == "core"].plot(ax=ax, aspect=None, color=col, alpha=0.45, edgecolor=col, linewidth=1.2, zorder=4)
    ...
    for i, fw in enumerate(EXTERNAL.values()):
        ...
        g.plot(ax=ax, aspect=None, facecolor="none", edgecolor="#222222", hatch=HATCH[i % len(HATCH)], linewidth=0.8, zorder=6)
        ext_handles.append(Patch(facecolor="none", edgecolor="#222222", hatch=HATCH[i % len(HATCH)],
                                 label=f"{fw.org} — {fw.label}" + ("" if fw.verified else " (unverified)")))

    if GLOFAS_POINT:
        ax.plot(*GLOFAS_POINT, marker="^", color="black", markersize=11, zorder=8)
        ax.annotate("GloFAS G5196\nAkokoro", GLOFAS_POINT, xytext=(6, 6), textcoords="offset points", fontsize=8)

    # labels
    for key, zone in ZONES.items():
        c = zone_districts(key, include_candidates=False).dissolve().geometry.iloc[0].representative_point()
        ax.annotate(zone.label.split(" (")[0], (c.x, c.y), ha="center", fontsize=9, fontweight="bold",
                    color=ZONE_COL[key], zorder=9,
                    bbox=dict(boxstyle="round,pad=0.2", fc="white", ec="none", alpha=0.75))

    zone_handles = [Patch(facecolor=ZONE_COL[k], alpha=0.45, edgecolor=ZONE_COL[k], label=f"{z.label} — core")
                    for k, z in ZONES.items()]
    zone_handles.append(Patch(facecolor="#888888", alpha=0.18, edgecolor="#888888", label="candidate districts (to be ruled in/out)"))
    if GLOFAS_POINT:
        zone_handles.append(Line2D([], [], marker="^", color="black", linestyle="", markersize=9, label="GloFAS reporting point G5196"))
    leg1 = ax.legend(handles=zone_handles, loc="lower left", fontsize=7.5, title="OCHA/CERF trigger zones", title_fontsize=8, framealpha=0.9)
    ax.add_artist(leg1)
    if ext_handles:
        ax.legend(handles=ext_handles, loc="lower right", fontsize=7.5, title="Other organisations' flood AA", title_fontsize=8, framealpha=0.9)

    ax.set_xlim(29.4, 35.2)
    ax.set_ylim(-1.6, 4.4)
    ax.set_aspect("equal")
    ax.set_axis_off()
    ax.set_title("Uganda flood anticipatory action — trigger zones and existing coverage", fontsize=12)
    fig.tight_layout()
